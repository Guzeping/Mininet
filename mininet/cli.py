...
class CLI(Cmd):
    ...
    def do_hostPing(self, line):
        ...
        if len(args) == 2:
            rate = args[0]
            during_time = args[1]
            self.mn.hostPing(rate=rate, duration_time=during_time)

    def do_netPing(self, line):
        args = line.split()
        if len(args) == 1:
            rate = args[0]
            self.mn.func(rate)
        if len(args) == 2:
            rate = args[0]
            during_time = args[1]
            self.mn.netPing(net_rate=rate, duration_time=during_time)

    def do_timePing(self,line):
        args=line.split()
        self.mn.timePing(args[0])
